const TYPE_COLOR = {
	...
	address: 'bg-gray-500/20 text-gray-300 border-gray-600/40',
		...
		<aside className="absolute right-6 top-6 z-10 w-full max-w-sm animate-in rounded-xl border border-gray-700 bg-gray-800 p-4 shadow-2xl">
			<div className="mb-3 flex items-start justify-between">
				...
				<button
					type="button"
					onClick={onClose}
					className="rounded-md border border-gray-700 px-2 py-1 text-sm text-gray-300 transition hover:bg-gray-700"
				>
					X
				</button>
			</div>
					...
					<tbody>
						{entries.map(([key, value]) => (
							<tr key={key} className="border-b border-gray-700 last:border-b-0">
								<td className="w-2/5 bg-gray-900 px-3 py-2 text-gray-400">{key}</td>
								<td className="break-all px-3 py-2 text-gray-100">{String(value)}</td>
							</tr>
						))}
					</tbody>
				</table>
